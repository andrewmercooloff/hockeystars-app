-- Финальная версия исправления ограничения уникальности для таблицы player_teams
...
-- Используем простой подход без сложных подзапросов
UPDATE player_teams 
...
WHERE id IN (
  ...
  WHERE EXISTS (
    ...
    WHERE pt2.player_id = pt1.player_id
      ...
      AND pt2.id != pt1.id
      AND pt2.start_year IS NOT NULL
  )
  AND pt1.start_year IS NOT NULL
);
